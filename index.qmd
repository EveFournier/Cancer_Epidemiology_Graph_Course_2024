---
title: "Cancer epidemiology in Geneva"
...
# The aim of this project is to analyse cancer evolution in Geneva's canton. It is divided in two sub-project :
  # - Interactive graphs per cancer subsites : 
      # main cancer type in male and female for period 2019-2021
      # evolution of main cancer types through the period 1989-2021
  # - Interactive chloroplet Map showing the evolution of cancer incidence in Geneva's canton by geographic unit
# files needed :
...
# Note 2 : There was a problem with shapefile found on internet.  errors with ggplotly => just a choroplet map, not interactive
# Note 3 : the standardisation used to compute incidence rates is 1972 european population
...
The maps below show the evolution of (made-up) incidence rates by area. To account for spatial dependencies and smooth the data to make patterns more visible, spatial smoothing techniques have to be used. A few approaches that can be considered are Kernel Smoothing, Spatial Moving Average , Bayesian Smoothing or Empirical Bayesian smoothing. These methods are needed to interpret the results and will be used in future analyses.
...
__Cancer evolution__

The data presented on this website are extracted from Geneva's cancer registry database (more about cancer registries on the following [YouTube page](https://youtu.be/zlV6BYoIwCA))
...
The maps presented on this website do not represent actual data and are intended for educational purposes only. The statistical method used, the population files used (no detailed five-year aged category, names of cities in full text instead of postal code) and the shape file do not meet scientific requirement. Those three axis need to be improved.

<br> <!-- Adding extra space -->
No interactive maps were generated because of the ggplotly error shown below.

The code that follows
<br> <!-- Adding extra space -->
interactive_map_incidence_2019_men<-ggplotly(map_incidence_2019_men)  
<br> <!-- Adding extra space -->
Generated the following unsolvable error
<br> <!-- Adding extra space -->
 Erreur dans st_coordinates.sfc(sf::st_geometry(model)) : 
   not implemented for objects of class sfc_GEOMETRY
...
-   Shapefile of Geneva's canton was downloaded from
    [Metabolism of Cities - Geneva Maps](https://data.metabolismofcities.org/dashboards/geneva/maps/578032/view/)

<br> <!-- Adding extra space -->

-   Population canton with time for three age categories was downloaded from the following websites:
    - [Statistiques cantonales page d'accueil](https://statistique.ge.ch/domaines/01/01_01/tableaux.asp#9)
    - [Fichier population 2013-2023](https://statistique.ge.ch/statistique/tel/domaines/01/01_01/T_01_01_12_04.xls)
    - [Fichier population 1989-2013](https://statistique.ge.ch/statistique/tel/domaines/01/01_01/T_01_01_12_04_1989-2013.xls)

<br> <!-- Adding extra space -->

-  Finding color range according to an initial color
    [HTML CSS Color](https://www.htmlcsscolor.com/hex/323452)
...
#load(file = here("environnement_backup_my_Rproject.RData"))
